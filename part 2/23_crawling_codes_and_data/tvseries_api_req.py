...
id_df = pd.read_csv("input_tvseries_ids.csv")
...
col_ind_totalSeasons = id_df.columns.get_loc("totalSeasons")
...
for title_id_i in id_df.tconst:
    ...
    if response_dict['Response'] != 'False' and (response_dict['Type'] != 'short' and response_dict['Type'] != 'movie'):
        ...
        id_df.iat[k, col_ind_totalSeasons] = response_dict['totalSeasons']
        #--------------------------------------------------------------------    
...
id_df.to_csv("output_tvseries_data.csv", columns = ['Title_id','Title','Plot','Awards', 'totalSeasons', 'Year', 'Rated', 'Runtime', 'Director', 'Actors', 'Language', 'Type'], index = False)
